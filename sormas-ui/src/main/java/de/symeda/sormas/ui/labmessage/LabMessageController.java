...
//					pickOrCreateEntry(labMessageDto, similarCases, similarContacts, similarEventParticipants);
	...
	private void pickOrCreateEntry(
		...
		selectionField.addCommitListener(() -> {
			...
			if (similarEntriesDto.isNewCase()) {
				...
				SampleCriteria caseSamplesCriteria = new SampleCriteria();
				caseSamplesCriteria.caze(caseDto.toReference());
				caseSamplesCriteria.setDisease(caseDto.getDisease());
				List<SampleDto> caseSamples = FacadeProvider.getSampleFacade().getByCaseUuids(Arrays.asList(caseDto.getUuid()));
				if (caseSamples.isEmpty()) {
					createSample(caseDto, labMessageDto);
				} else {
					pickOrCreateSample(caseDto, labMessageDto, caseSamples);
				}
			}
		});
	...
	private void pickOrCreateSample(CaseDataDto caseDataDto, LabMessageDto labMessageDto, List<SampleDto> samples) {
		...
		selectionField.addCommitListener(() -> {
			...
			if (sampleDto != null) {
				...
				createSample(caseDataDto, labMessageDto);
			}
		});
		...
		showFormWithLabMessage(labMessageDto, selectionField, window, I18nProperties.getCaption(Strings.headingPickOrCreateSample));
	}
